...
func getSockOptAddrInet4(fd, level, opt int) (*syscall.RawSockaddrInet4, error) {
	var value syscall.RawSockaddrInet4
	vallen := sockLen(syscall.SizeofSockaddrInet4)
	err := getSockOpt(fd, level, opt, unsafe.Pointer(&value), &vallen)
	return &value, err
}

func getSockOriginalDst(conn net.Conn) (*net.TCPAddr, error) {
	const SO_ORIGINAL_DST int = 80

	tcpConn := conn.(*net.TCPConn)

	connValue := reflect.ValueOf(tcpConn).Elem()
	fdValue := connValue.FieldByName("fd")
	if !fdValue.IsValid() {
		return nil, fmt.Errorf("get fd from net.TCPConn err")
	}

	sysfdValue := fdValue.Elem().FieldByName("sysfd")
	if !sysfdValue.IsValid() {
		return nil, fmt.Errorf("get sysfd from net.conFd err")
	}
	sockAddr, err := getSockOptAddrInet4(int(sysfdValue.Int()), syscall.IPPROTO_IP, SO_ORIGINAL_DST)
	if err != nil {
		return nil, os.NewSyscallError("getsockopt", err)
	}
	tcpAddr := &net.TCPAddr{IP: sockAddr.Addr[:], Port: int((sockAddr.Port >> 8) | (sockAddr.Port << 8))}
